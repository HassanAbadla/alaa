        ...
        <template v-if="!loading">
          <v-row>
            <v-col cols="6">
              ...
              <!-- Temporarily Commented for Menus error  -->
              <!-- <custom-dropdown
...
export default {
  ...
  data() {
    return {
      ...
      users: [],
      subscriptionPackages: [1, 2, 3, 77, 78, 79, 80, 81, 82, 83],
      // projects: [],
      // buildings: [],
      // categories: [],
      // priorities: [],
  ...
  methods: {
    ...
    getNextNumber() {
      ...
      const existingCodes = this.$store.state.oss.orders.map((order) => order.code)
    ...
    async submit() {
      try {
        ...
        let data = {
          ...
          period: this.period,
          code: this.code,
          orders_status_id: this.$auth.user.type.code === "b-owner" ? 5 : 1, // 5 = Request, 1 = Draft
          ...
          // orders_list_id: this.$auth.user.type.code === "tasks" ? this.tssColumnId : null
        }
        if (this.$auth.user.type.code === "tasks") {
  ...
  computed: {
    ...mapState("flowOss", ["orders", "projects", "buildings", "categories", "priorities", "types", "statuses"]),
