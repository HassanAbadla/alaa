...
export default {
  ...
  computed: {
    ...mapGetters({
      getCurrentVisitStatus: "srvVisitStatuses/getCurrentVisitStatus",
  ...
  methods: {
    ...mapActions({
      addVisitStatus: "srvVisitStatuses/addVisitStatus",
      updateVisitStatus: "srvVisitStatuses/updateVisitStatus",
      fetchVisitStatusById: "srvVisitStatuses/fetchVisitStatusById"
    }),
    ...mapActions("drawer", ["closeDrawer"]),
    async handleSubmit() {
      const computedCode = this.formData.name.slice(0, 3).toLowerCase()
      this.formData.code = computedCode
      this.loadingVisitStatuses = true
      if (this.editForm) {
        this.formData.id = this.itemId
        await this.updateVisitStatus(this.formData)
      } else {
        await this.addVisitStatus(this.formData)
      }
      this.loadingVisitStatuses = false
      this.closeDrawer()
    }
  },
  async mounted() {
    if (this.editForm) {
      await this.fetchVisitStatusById(this.itemId)
      this.formData.name = this.getCurrentVisitStatus.name
      this.formData.note = this.getCurrentVisitStatus.note
      this.formData.color = this.getCurrentVisitStatus.color || "#1976D2"
    }
  }
}
</script>
