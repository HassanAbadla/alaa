...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      getCurrentRating: "tprmRatings/getCurrentRating",
      loadingRating: "tprmRatings/isLoading"
    }),
    swatchStyle() {
      return {
  ...
  methods: {
    ...mapActions({
      addRating: "tprmRatings/addRating",
      updateRating: "tprmRatings/updateRating",
      fetchRatingById: "tprmRatings/fetchRatingById"
    }),
    ...
    async handleSubmit() {
      ...
      if (this.editForm) {
        ...
        await this.updateRating(this.formData)
      } else {
        await this.addRating(this.formData)
      }
  ...
  async mounted() {
    if (this.editForm) {
      await this.fetchRatingById(this.itemId)
      this.formData.name.en = this.getCurrentRating.name.en
      this.formData.name.ar = this.getCurrentRating.name.ar
      this.formData.note = this.getCurrentRating.note
      this.formData.color = this.getCurrentRating.color || "#1976D2"
    }
  }
}
</script>
