<template>
  <div class="gantt-chart card">
    <div class="d-flex border-bottom">
      ...
      <div class="order-details-column bg-light border-right p-2 font-weight-bold">{{ $t("page.orders") }}</div>
      <!-- Timeline header -->
    ...
    <div v-if="sortedOrders.length > 0" class="d-flex">
      <!-- Left side fixed column -->
      <div class="order-details-column border-right">
        <div
          v-for="order in sortedOrders"
          :key="order.id"
          @click="openDetails(order)"
          class="d-flex flex-column justify-space-between pa-2 border-bottom border"
        >
          <div class="font-weight-bold small">
            {{ order.name || `Order #${order.id}` }}
          </div>
          <!-- <div class="d-flex small text-muted justify-space-between">
            <div class="text-muted smaller">Created: {{ formatDate(order.created_at) }}</div>
            <div class="text-muted smaller">Expected: {{ formatDate(order.date_expected) }}</div>
          </div> -->
        </div>
      </div>
        ...
        <div class="timeline-wrapper" :style="{ width: timelineWidth + 'px', minHeight: '100%' }">
          ...
          <div class="bars-container">
            <div v-for="(order, index) in sortedOrders" :key="order.id" class="timeline-row">
              ...
              >
                <div class="bar-label">{{ getDurationDays(order) }} {{ $t("page.days") }}</div>
...
import OrderGanttDetails from "./OrderGanttDetails.vue"
export default {
  name: "OrderGanttChart",
  ...
  computed: {
    ...
    sortedOrders() {
      return [...this.orders].sort(
        (a, b) => this.parseDate(a.created_at).getTime() - this.parseDate(b.created_at).getTime()
      )
    },
    ...
    startDate() {
      ...
      const dates = this.sortedOrders.map((order) => this.parseDate(order.created_at))
