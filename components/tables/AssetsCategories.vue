  ...
  <v-can show-message :permission="'create_policy'">
    <v-card flat class="pt-12">
      <v-card-text class="pt-0">
        <custom-table
          :title="$t('page.assets_categories')"
          :headers="headers"
          :tableConfig="tableConfig"
          :propItems="categoriesList"
          :propLoading="loadingCategories"
          hideView
          :filters.sync="filters"
          @clear-filters="fetchCategories"
          :sortOnLoad="false"
          hideHeaders
          :fullPageTable="false"
          class="table-fill"
...
export default {
  ...
  data() {
    return {
      tableConfig: {
        delete: this.deleteAssetCategories,
        itemPath: "/settings/asset-categories",
        createForm: {
  ...
  methods: {
    ...mapActions("assetsCategories", ["createAssetCategory", "fetchCategories", "deleteAssetCategories"]),
    ...
    openEditDialog(item) {
      ...
      this.openDrawer({
        component: "forms/PolicyCreateForm",
        props: {
          item: item,
          editForm: true
        },
