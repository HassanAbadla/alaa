  ...
  <div class="gantt-chart card">
    ...
    <div class="d-flex justify-content-end mb-2">
      <div class="btn-group" role="group">
        <button
          type="button"
          class="btn btn-sm"
          :class="viewMode === 'days' ? 'btn-primary' : 'btn-outline-primary'"
        ...
        <button
          type="button"
          class="btn btn-sm"
          :class="viewMode === 'weeks' ? 'btn-primary' : 'btn-outline-primary'"
    ...
    <div class="d-flex border-bottom">
      ...
      <div v-if="timelineDates" class="timeline-container" ref="headerScroll">
        <div class="d-flex" :style="{ width: timelineWidth + 'px' }">
          ...
          >
            {{ formatTimelineHeader(date, index) }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="orders.length > 0" class="d-flex">
      ...
      <div class="border-right">
        <div v-for="order in orders" :key="order.id" @click="openDetails(order)" class="d-flex border-bottom border">
          <div
      ...
      <div class="timeline-container" ref="contentScroll">
        <div class="timeline-wrapper" :style="{ width: timelineWidth + 'px', minHeight: '100%' }">
          ...
          <div class="bars-container">
            <div v-for="(order, index) in orders" :key="order.id" class="timeline-row">
              <div
                class="gantt-bar"
...
export default {
  ...
  computed: {
    ...
    dayWidth() {
      return this.viewMode === "days" ? 38.2 : 83.5 // 83.5px for week view
    },
    ...
    timelineDates() {
      ...
      if (this.viewMode === "days") {
        ...
        // Week view - generate week start dates
        let current = new Date(start)
    ...
    startDate() {
      if (!this.orders.length) return new Date()

      // Get the earliest date_expected date
      const dates = this.orders.map((order) => this.parseDate(order.date_expected))
    ...
    endDate() {
      ...
      const dates = this.orders.map((order) => this.parseDate(order.date_promised))
    ...
    getTodayPosition() {
      ...
      if (this.viewMode === "days") {
        diffDays = this.getDaysBetween(this.startDate, this.today)
      } else {
        // Week view - calculate position within weeks
        const diffInDays = this.getDaysBetween(this.startDate, this.today)
        diffDays = diffInDays / 7 // Convert to weeks
      }
  ...
  methods: {
    ...
    getOrderBarStyle(order) {
      ...
      let startPosition, barWidth

      if (this.viewMode === "days") {
        const startDays = this.getDaysBetween(this.startDate, orderStart)
        const duration = this.getDurationDays(order)
        startPosition = startDays * this.dayWidth
        barWidth = Math.max(1, duration) * this.dayWidth
      } else {
        // Week view
        const startDays = this.getDaysBetween(this.startDate, orderStart)
        const duration = this.getDurationDays(order)
        startPosition = (startDays / 7) * this.dayWidth
        barWidth = Math.max(1, duration / 7) * this.dayWidth
      }

      console.log("left:", `${startPosition}px`)
    ...
    getMilestonePosition(dateStr, order) {
      ...
      let position
      if (this.viewMode === "days") {
        position = (days + fractionalDay) * this.dayWidth
      } else {
        position = ((days + fractionalDay) / 7) * this.dayWidth
      }
